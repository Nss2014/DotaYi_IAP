...
//    }else{
    ...
    NSLog(@"%f   %f  %f",menu.frame.size.width,touchPoint.x,touchPoint.y);
    
    NSLog(@"CGRectContainsPoint %d",CGRectContainsPoint(menu.frame, CGPointMake(0, 0)));
...
- (void)setMenu:(UIView *)menu animation:(BOOL)animated{
    ...
    {
        topMenu.frame = (CGRect){CGPointMake(0, self.bounds.size.height - topMenu.bounds.size.height/2), topMenu.bounds.size.width,topMenu.bounds.size.height/2};
    }
    else
    {
...
- (void)dismissGridMenu:(SGBaseMenu *)menu Animated:(BOOL)animated
{
...
+ (void)showNSSGridMenuWithTitle:(NSString *)title
                   itemTitles:(NSArray *)itemTitles
                       images:(NSArray *)images
               selectedHandle:(SGMenuActionHandler)handler{
    SGGridMenu *menu = [[SGGridMenu alloc] initWithTitle:title
                                              itemTitles:itemTitles
                                                  images:images];
    [menu triggerSelectedAction:^(NSInteger index){
        [[SGActionView sharedActionView] dismissGridMenu:menu Animated:YES];
