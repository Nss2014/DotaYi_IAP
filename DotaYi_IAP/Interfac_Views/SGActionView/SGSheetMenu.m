...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        {
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 20) style:UITableViewStylePlain];
        }
        else
        {
...
//    }
    ...
    if([deviceStr rangeOfString:@"iPad"].location != NSNotFound)
    {
        self.tableView.frame = CGRectMake(self.bounds.size.width * 0.05, height, self.bounds.size.width * 0.9, SCREEN_HEIGHT - SCREEN_HEIGHT/5 - height);
        
        
        self.bounds = (CGRect){CGPointZero, CGSizeMake(self.bounds.size.width, SCREEN_HEIGHT - SCREEN_HEIGHT/5)};
    }
    else
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        UILabel *cusTextlabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width * 0.9, cell.contentView.frame.size.height)];
        
        cusTextlabel.textAlignment = NSTextAlignmentCenter;
        
        cusTextlabel.textColor = BaseMenuTextColor(self.style);
        
        cusTextlabel.backgroundColor = CLEAR_COLOR;
        
        [cell.contentView addSubview:cusTextlabel];
        
        cusTextlabel.text = self.items[indexPath.row];
        
        if (self.selectedItemIndex == indexPath.row) {
            //        cell.accessoryType = UITableViewCellAccessoryCheckmark;
            
            cusTextlabel.textColor = XLS_COLOR_MAIN_GREEN;
        }else{
            cell.accessoryType = UITableViewCellAccessoryNone;
        }
    }
