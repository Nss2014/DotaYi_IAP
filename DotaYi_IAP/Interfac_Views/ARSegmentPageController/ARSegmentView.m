...
-(void)_baseConfigs
{
    ...
    _segbackView = [[UIView alloc] init];
    
    _segbackView.backgroundColor = WHITE_COLOR;
    
    [self addSubview:self.segbackView];
...
//    _segmentControl.tintColor = SEPRATELINE_GRAYCOLOR;
    ...
    [self.segbackView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.segmentControl mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(ws.segbackView.mas_left).offset(-5);
        make.centerY.equalTo(ws.segbackView.mas_centerY);
        make.height.equalTo(ws.segbackView.mas_height);
        make.right.equalTo(ws.segbackView.mas_right).offset(5);
    }];
    
    self.segbackView.clipsToBounds = YES;
