...
- (void)loadView {
    ...
    CGFloat um_height = _feedbackView.frame.size.height - 44 - 0;
    if (self.modalStyle) {
        self.mTableView = [[UITableView alloc]
                           initWithFrame:CGRectMake(0, 0, _feedbackView.frame.size.width, um_height)];
    } else {
        self.mTableView = [[UITableView alloc]
                           initWithFrame:CGRectMake(0, 0, _feedbackView.frame.size.width, um_height)];
    }
    ...
    switch (orientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown: {
            ...
            if (self.modalStyle) {
                ...
                frame.origin.y = _feedbackView.frame.size.height - frame.size.height -NAV_HEIGHT;
            }
            ...
            if (self.modalStyle) {
                self.mTableView.frame = CGRectMake(0, inputToolbarHeight + 20, viewWidth, _feedbackView.frame.size.height - inputToolbarHeight -NAV_HEIGHT);
            } else {
                self.mTableView.frame = CGRectMake(0, 0, viewWidth, _feedbackView.frame.size.height - inputToolbarHeight - NAV_HEIGHT);
            }
            //标记 frame修改
        ...
        case UIInterfaceOrientationLandscapeRight: {
            ...
            if (self.inputToolBar.isEditMode) {
                self.inputToolBar.frame = CGRectMake(0, _feedbackView.frame.size.height - 82 - NAV_HEIGHT , viewWidth, 82);
            } else {
...
- (void)keyboardAction:(NSNotification *)aNotification isShow:(BOOL)isShow {
    ...
    {
        frame.origin.y = (_feedbackView.frame.size.height - (isShow ? keyboardEndFrame.size.height : 0)) - self.inputToolBar.frame.size.height - NAV_HEIGHT;
    } else {
        if (isShow) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (content.length > 0) {
       CGSize labelSize = [Tools getAdaptionSizeWithText:content andFont:[UIFont systemFontOfSize:14.0] andLabelWidth:self.mTableView.frame.size.width - 40];
